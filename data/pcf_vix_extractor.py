            ...
            if all(col in df.columns for col in ['ETF Code', 'ETF Name']):
                ...
                if not holdings_found:
                    ...
                    patterns = [
                        ...
                        # Pattern for direct VXH5 format
                                    ...
                                    if len(match.groups()) == 1 and match.group(1).isdigit() and len(match.group(1)) == 4:
                                        ...
                                        if len(code) == 4:
                                            ...
                                            if month in month_map:
                                                ...
                                                pcf_ticker = f"PCF:VX{month_letter}{year}"
                                                std_ticker = f"/VX{month_letter}{year}"
...
def save_pcf_data(futures_data, save_dir=SAVE_DIR):
    """Save PCF futures data as CSV"""
    if not futures_data or len(futures_data) <= 2:
        logger.warning("No PCF futures data to save")
        return None
    
    try:
        # Format data into standardized records
        records = format_vix_data(futures_data, "PCF")
        ...
        # Save to CSV
        timestamp = datetime.now().strftime("%Y%m%d%H%M")
        csv_filename = f"vix_futures_pcf_{timestamp}.csv"
        csv_path = os.path.join(save_dir, csv_filename)
        
        df.to_csv(csv_path, index=False)
        logger.info(f"Saved PCF futures data to {csv_path}")
        
        return csv_path
    
    except Exception as e:
        logger.error(f"Error saving PCF data: {str(e)}")
        logger.error(traceback.format_exc())
        return None

if __name__ == "__main__":
    # Extract VIX futures from PCF
    pcf_data = extract_vix_futures_from_pcf()
    
    # Save to CSV if data was found
    if pcf_data:
        csv_path = save_pcf_data(pcf_data)
        if csv_path:
            print(f"✅ PCF VIX futures data saved to: {csv_path}")
        else:
            print("❌ Failed to save PCF data")
            exit(1)
    else:
        print("❌ No VIX futures data found in PCF file")
        exit(1)
, '').replace(',', '')
...
def extract_vix_patterns_from_all_cells(df):
    ...
    patterns = [
        ...
        re.compile(r'VX([A-Z])(\d{1,2})', re.IGNORECASE)   # VXH5
    ]
